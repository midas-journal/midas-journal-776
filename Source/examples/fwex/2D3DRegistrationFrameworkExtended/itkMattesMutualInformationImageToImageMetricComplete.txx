...
/*=========================================================================
  ...
  Module:    $RCSfile: itkMattesMutualInformationImageToImageMetricComplete.txx,v $
  Language:  C++
...
#ifndef _itkMattesMutualInformationImageToImageMetricComplete_txx
#define _itkMattesMutualInformationImageToImageMetricComplete_txx
...
//#include "itkOptMattesMutualInformationImageToImageMetricComplete.txx"
//#else
// --> forget multithreaded mode!
...
MattesMutualInformationImageToImageMetricComplete<TFixedImage,TMovingImage>
...
::Initialize(void) throw ( ExceptionObject )
{
  ...
  //CHNG: apply lower/upper bound if it is defined for fixed image (2nd item);
  // otherwise search for min/max intensities in fixed image:
  if (m_LowerBound.size() < 2 || m_UpperBound.size() < 2)
  ...
  { // simply apply the specified bounds:
    fixedImageMin = m_LowerBound[1];
    fixedImageMax = m_UpperBound[1];
  }
  ...
  //CHNG: apply lower/upper bound if it is defined for moving image (1st item);
  // otherwise search for min/max intensities in moving image (which is not
  // meaningful in general for 2D/3D-registrations):
  if (m_LowerBound.size() < 1 || m_UpperBound.size() < 1)
  ...
  { // simply apply the specified bounds:
    movingImageMin = m_LowerBound[0];
    movingImageMax = m_UpperBound[0];
  }

  m_MovingImageTrueMin = movingImageMin;
  m_MovingImageTrueMax = movingImageMax;

  //CHNG: write debug information if bounds were overridden
  if (m_LowerBound.size() > 0 && m_UpperBound.size() > 0)
  { if (m_LowerBound.size() > 1 && m_UpperBound.size() > 1)
    { itkDebugMacro(" * MovingImageMin, MovingImageMax, FixedImageMin, " <<
        "FixedImageMax were overridden by user-specified bounds!");
    }
    else
    { itkDebugMacro(" * MovingImageMin, MovingImageMax " <<
        "were overridden by user-specified bounds!");
    }
  }
 ...
 */
template < class TFixedImage, class TMovingImage  >
typename MattesMutualInformationImageToImageMetricComplete<TFixedImage,TMovingImage>
::MeasureType
MattesMutualInformationImageToImageMetricComplete<TFixedImage,TMovingImage>
::GetValue( const ParametersType& parameters ) const
{
  std::cerr << "GetValue: " << parameters << "\n";
...
// NOTE: uncommented this exception-block: it is the user's responsibility
// to provide adequate intensity-bounds, fixed image region and 
...
*/
  ...
  //if (nSamples < m_NumberOfSpatialSamples / 4)
  //  std::cerr << "::GetValue(): " << nSamples << " (" << 
  //    m_NumberOfSpatialSamples << ")" << std::endl;
...
*/
  ...
  //if (nSamples < m_NumberOfSpatialSamples / 4)
  //  std::cerr << "::GetValueAndDerivative(): " << nSamples << " (" << 
  //    m_NumberOfSpatialSamples << ")" << std::endl;
...
::GetDerivative( const ParametersType& parameters,
                 DerivativeType & derivative ) const
{
  std::cerr << "GetDerivative: " << parameters << "\n";
  MeasureType value;
